...
#função para carregarodataset
@st.cache
def get_data():
      return pd.read_csv("c:/projetodsia/marcilio-data.csv")

#função para treinaromodelo
def train_model():
     data=get_data()
...
#subtítulo
st.markdown("Este é um Data App utilizado para exibirasolução de Machine Learning paraoproblema de predição de valores de imóveis de Boston.")
...
#defindo atributosapartir do multiselect
...
chas=st.sidebar.selectbox("Faz limite comorio?",("Sim","Não"))
#transformandoodado de entrada em valor binário
...
1stat=st.sidebar.number_input("Porcentagem de status baixo",value=data.LSTAT.mean())

#inserindo um botão na tela
btn_predict=st.sidebar.button("Realizar Predição")

#verifica seobotão foi acionado
if btn_predict:
   result = model.predict([[crim,indus,chas,nox,rm,ptratio,b,lstat]])
   st.subheader("Ovalor previsto paraoimóvelé:")
